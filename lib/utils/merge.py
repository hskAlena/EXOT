...
def merge_template_search_odin(inp_list, return_search=False, return_template=False):
    """NOTICE: search region related features must be in the last place"""
    seq_dict = {"feat": torch.cat([x["feat"] for x in inp_list], dim=0).requires_grad_(),
                "mask": (torch.cat([x["mask"] for x in inp_list], dim=1)/1.).requires_grad_(),
                "pos": torch.cat([x["pos"] for x in inp_list], dim=0).requires_grad_()}
    if return_search:
        x = inp_list[-1]
        seq_dict.update({"feat_x": x["feat"], "mask_x": x["mask"], "pos_x": x["pos"]})
    if return_template:
