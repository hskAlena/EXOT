...
class Sequence:
    ...
    def __init__(self, name, frames, dataset, ground_truth_rect, ground_truth_seg=None, init_data=None,
                 object_class=None, target_visible=None, object_ids=None, obj_name=None, multiobj_mode=False):
        self.name = name
        self.frames = frames
        self.dataset = dataset
        self.ground_truth_rect = ground_truth_rect
        self.ground_truth_seg = ground_truth_seg
        self.ground_truth_objname = obj_name
    ...
    def object_init_data(self, frame_num=None) -> dict:
        # print("Where to find? ", frame_num)
        if frame_num is None:
            frame_num = 0
        if frame_num not in self.init_data:
            init_data = dict()
            init_data['object_class'] = self.object_class
            init_data['object_name'] = self.ground_truth_objname   
            # print("inside", self.object_class, self.ground_truth_objname)         
            try:
                init_data['target_visible'] = self.target_visible[frame_num]
            except:
                print(frame_num, self.target_visible.shape)
                init_data['target_visible'] = self.target_visible[frame_num-1]
            return init_data
        ...
        # print(self.object_class, self.target_visible)
        init_data['object_class'] = self.object_class
        init_data['target_visible'] = self.target_visible[frame_num] 
        # print("ISTIS", init_data['object_class'], init_data['target_visible'])     
        return init_data
