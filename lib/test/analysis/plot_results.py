...
def print_results(trackers, dataset, report_name, merge_results=False,
    ...
    if 'success' in plot_types:
        ...
        # print("AUC", auc, auc.shape)
        scores['EXIST'] = torch.mean(torch.tensor(eval_data['avg_fpfn_all'])[:,:,0], dim=0)
        scores['EXIT'] = torch.mean(torch.tensor(eval_data['avg_fpfn_all'])[:,:,1], dim=0)
        # print(scores['EXIST'], scores['EXIST'].shape)
    ...
    # ********************************  TNR AUROC Plot *********************************
    if 'main_tnr_auroc' in plot_types:
        main_tnr_auroc = torch.tensor(eval_data['main_tnr_auroc'])
        tnr_avg = torch.mean(main_tnr_auroc[:,:,0], dim=0)
        auroc_avg = torch.mean(main_tnr_auroc[:,:,1], dim=0)

        scores['Main FPR'] = 1-tnr_avg
        scores['Main AUROC'] = auroc_avg
