...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                ...
                if load_previous_ckpt:
                    print(self.settings.st1_name, self.settings.resume, self.settings.resume_name)
                    directory = '{}/{}'.format(self._checkpoint_dir, self.settings.project_path_prv)
                    self.load_state_dict(directory, st1_name=self.settings.st1_name, resume_name=self.settings.resume_name)
                if distill:
    ...
    def load_state_dict(self, checkpoint=None, st1_name=None, resume_name=None, distill=False):
        """Loads a network checkpoint file.
        ...
        if isinstance(checkpoint, str):
            # checkpoint is the path          
            if os.path.isdir(checkpoint):
                upfolderpath = '/'.join(checkpoint.split('/')[:-1])
                if resume_name != 'None':
                    print(resume_name, "WHY resume")
                    checkpoint_path = os.path.join(upfolderpath, resume_name)
                elif st1_name != 'None':
                    print(st1_name, "WHY st1 name")
                    checkpoint_path = os.path.join(upfolderpath, st1_name)
                else:
                    checkpoint_list = sorted(glob.glob('{}/*_ep*.pth.tar'.format(checkpoint)))
                    if checkpoint_list:
                        checkpoint_path = checkpoint_list[-1]
        ...
        print("WARNING: Assert negleccted!!")
        # assert net_type == checkpoint_dict['net_type'], 'Network is not of correct type.'
