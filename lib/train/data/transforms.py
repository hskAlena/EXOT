...
class Resize(TransformBase):
    """Normalize image"""
    def __init__(self, height=256, width=456, interpolation=tvisf.InterpolationMode.BILINEAR):
        super().__init__()
        self.width = width
        self.height = height
    ...
    def transform_image(self, image):
        ...
        if image.shape[-1]==self.width:
            return image
        return tvisf.resize(image, [self.height, self.width], self.interpolation)

    def transfrom_mask(self, mask):
        print(mask.shape)
        if mask.shape[-1]==self.width:
            return mask
        return tvisf.resize(mask, [self.height, self.width], self.interpolation)

    def transform_att(self, att):
        print(att.shape)
        if att.shape[-1]==self.width:
            return att
        return tvisf.resize(att, [self.height, self.width], self.interpolation)
