...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, batch_size = 16):
        """
        args:
    ...
    def getitem_joint(self, index):
        ...
        while not valid:
            ...
            seq_id, visible, seq_info_dict = self.sample_seq_from_dataset(dataset, is_video_dataset, idx)
            ...
            try:
                ...
                if dataset.get_name() == 'robot':
                    template_depth = meta_obj_train['depth']
                    search_depth = meta_obj_test['depth']
                    template_joint = meta_obj_train['joint']
                    search_joint = meta_obj_test['joint']
                    joint_flag = meta_obj_test.get('joint_flag')
                    exit_flag = meta_obj_test['exit_flag']
                else:
                    template_depth = [torch.zeros((H, W))] * self.num_template_frames
                    search_depth = [torch.zeros((H, W))] * self.num_search_frames
                    template_joint = [torch.zeros(6)] * self.num_template_frames
                    search_joint = [torch.zeros(6)] * self.num_search_frames
                    joint_flag = 'None'
                    exit_flag = [torch.zeros(4)]* self.num_search_frames
                
                data = TensorDict({'template_images': template_frames,
                                   'template_anno': template_anno['bbox'],
                                   'template_masks': template_masks,
                                   'template_joint': template_joint,
    ...
    def getitem(self, index):
        ...
        while not valid:
            ...
            try:
                ...
                if dataset.get_name() == 'robot' or dataset.get_name() == 'trek150':
                    exit_flag = meta_obj_test['exit_flag']
                else:
                    exit_flag = [torch.zeros(1)]* self.num_search_frames
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset, idx=None):
        ...
        while not enough_visible_frames:
            ...
            # else:
            #     seq_id = idx % dataset.get_num_sequences()
